...
/**
 * Bir öğretmenin, yapay zekanın önerdiği (AI_SUGGESTED) bir etüt seansını
 * onaylamasını/reddetmesini sağlar — taksit tahsilatı API'sinde kurulan
...
export async function POST(
  ...
  const outcome = await withTenantContext(actor, async (tx) => {
    ...
    if (session.status !== StudySessionStatus.AI_SUGGESTED) {
      return { kind: "already_responded" as const, status: session.status };
    }

    const newStatus = decision === "APPROVE" ? StudySessionStatus.TEACHER_APPROVED : StudySessionStatus.TEACHER_REJECTED;
    const updateResult = await tx.studySession.updateMany({
      where: { id: session.id, status: StudySessionStatus.AI_SUGGESTED },
